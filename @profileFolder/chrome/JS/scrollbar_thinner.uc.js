...
// modded by Dustee to affect the main rightside scrollbar only
...
(function () {
    ...
    var css = `
		...
		:not(select):not(hbox) > scrollbar[orient="vertical"][root="true"] {
		...
		:not(select):not(hbox) > scrollbar[orient="vertical"][root="true"] thumb {
			-moz-appearance: none!important;
			background-color: transparent;
			pointer-events: auto;
		}
		...
		:not(select):not(hbox) > scrollbar[orient="vertical"][root="true"]:hover {
			background-color: rgba(0, 0, 0, 0.25);
			max-width: 12px!important;
			point-events: auto;
		}
		:not(select):not(hbox) > scrollbar[orient="vertical"][root="true"]:hover thumb {
			border-width: 12px;
			transition: border 0s linear;
		}
		:not(select):not(hbox) > scrollbar[orient="vertical"][root="true"] scrollbarbutton, :not(select):not(hbox) > scrollbar gripper {
			display: none;
		}
		@-moz-document domain("youtube.com") {
			:not(select):not(hbox) > scrollbar[orient = "vertical"][root="true"]:not(:hover) thumb {
				opacity: 0 !important;
			}
		}
	`;
